...
<p align="center">
  <a href="https://github.com/bharathvbcr/DevPrism">官网</a> ·
  <a href="https://github.com/bharathvbcr/DevPrism/releases/latest/download/DevPrism-macOS.dmg">macOS (Apple Silicon) をダウンロード</a> ·
  <a href="https://github.com/bharathvbcr/DevPrism/releases/latest/download/DevPrism-macOS-Intel.dmg">macOS (Intel) をダウンロード</a> ·
  <a href="https://github.com/bharathvbcr/DevPrism/releases/latest/download/DevPrism-Windows-setup.exe">Windows 版をダウンロード</a> ·
  <a href="https://github.com/bharathvbcr/DevPrism/releases/latest/download/DevPrism-Linux.AppImage">Linux 版をダウンロード</a> ·
  <a href="https://github.com/bharathvbcr/DevPrism/releases">所有版本</a>
</p>
...
DevPrism は**ローカルファースト**の代替ツールです — ファイルはローカルディスクに保存し、オフラインでコンパイルします。AI 機能は Gemini API 経由で実行するか、Ollama を使用してローカルで実行します。
...
DevPrism には、高速な Python パッケージマネージャである [uv](https://docs.astral.sh/uv/) が直接組み込まれています。ワンクリックで uv をインストールし、ワンクリックでプロジェクトごとの仮想環境を作成できます。Dev Engine は Python コードの実行時に `.venv` を自動で使用するため、エディタを離れることなく、グラフの生成、解析スクリプトの実行、データ処理を行えます。
...
スキルはグローバル（`~/.devprism/skills/`）またはプロジェクト単位でインストールでき、必要に応じてアシスタントが自動的に読み込みます。
...
保存するたびに、ローカル Git リポジトリ（`.devprism/history.git/`）にスナップショットが作成されます。重要なチェックポイントにラベルを付けたり、任意の 2 つのスナップショット間の diff を確認したり、以前のバージョンに復元したりできます。アシスタントが編集を提案すると、専用パネルに視覚的な差分とともに表示され、チャンクごとに承認・却下、または一括で適用・取り消し（`⌘Y` / `⌘N`）できます。
...
- **Slash Commands** ── 組み込み（`/review`, `/init`）+ `.devprism/commands/` のカスタムコマンド
