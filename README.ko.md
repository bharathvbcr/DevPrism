...
DevPrism은 **로컬 우선** 대안입니다 — 파일은 로컬 디스크에 저장하고 오프라인으로 컴파일합니다. AI 기능은 Gemini API를 통해 실행되거나 Ollama를 통해 로컬에서 실행됩니다.
...
### Python 환경 (uv)
DevPrism은 빠른 Python 패키지 관리자인 [uv](https://docs.astral.sh/uv/)를 앱에 직접 통합합니다. 원클릭으로 uv 설치, 원클릭으로 프로젝트 수준 가상 환경 생성. Dev Engine이 Python 코드 실행 시 `.venv`를 자동으로 사용하므로, 에디터를 떠나지 않고 플롯 생성, 분석 스크립트 실행, 데이터 처리가 가능합니다.
...
스킬은 전역(`~/.devprism/skills/`) 또는 프로젝트별로 설치되며, 어시스턴트가 관련 시점에 자동으로 로드합니다.
...
### 히스토리 & 변경 제안 검토
저장할 때마다 로컬 Git 저장소(`.devprism/history.git/`)에 스냅샷이 생성됩니다. 중요한 체크포인트에 라벨을 달고, 두 스냅샷 간의 diff를 탐색하고, 이전 버전을 복원할 수 있습니다. 어시스턴트가 편집을 제안하면 전용 패널에서 시각적 diff와 함께 표시되며 — 청크별로 수락/거부하거나 한 번에 모두 적용/취소(`⌘Y` / `⌘N`)할 수 있습니다.
...
- **슬래시 명령어** — 내장(`/review`, `/init`) + `.devprism/commands/`의 커스텀 명령어.
