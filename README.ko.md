...
<p align="center">
  <img src="./assets/demo/main.webp" alt="DevPrism 데모" width="800" />
</p>
...
DevPrism은 **로컬 우선** 대안입니다 — 파일은 로컬 디스크에 저장하고 오프라인으로 컴파일합니다. AI 기능은 Anthropic API로 콘텐츠를 전송하여 추론합니다 ([데이터 사용 정책](https://code.claude.com/docs/en/data-usage) 참고).
...
### Python 환경 (uv)
DevPrism은 빠른 Python 패키지 관리자인 [uv](https://docs.astral.sh/uv/)를 앱에 직접 통합합니다. 원클릭으로 uv 설치, 원클릭으로 프로젝트 수준 가상 환경 생성. Claude Code가 Python 코드 실행 시 `.venv`를 자동으로 사용하므로, 에디터를 떠나지 않고 플롯 생성, 분석 스크립트 실행, 데이터 처리가 가능합니다.
...
### 템플릿 & 프로젝트 마법사로 빠른 시작
템플릿(논문, 학위 논문, 프레젠테이션, 포스터, 서신 등)을 선택하고 이름을 지정한 후, 선택적으로 작성 내용을 설명하면 — DevPrism이 프로젝트를 설정하고 AI로 초기 콘텐츠를 생성합니다. 참고 파일(PDF, BIB, 이미지)을 드래그 앤 드롭하여 바로 작성을 시작하세요.
...
### 히스토리 & 변경 제안 검토
저장할 때마다 로컬 Git 저장소(`.claudeprism/history.git/`)에 스냅샷이 생성됩니다. 중요한 체크포인트에 라벨을 달고, 두 스냅샷 간의 diff를 탐색하고, 이전 버전을 복원할 수 있습니다. Claude가 편집을 제안하면 전용 패널에서 시각적 diff와 함께 표시되며 — 청크별로 수락/거부하거나 한 번에 모두 적용/취소(`⌘Y` / `⌘N`)할 수 있습니다. 히스토리는 스스로 크기를 관리합니다 — 스냅샷이 800개를 넘으면 자동으로 압축하여 라벨 붙은 체크포인트는 영원히 보존하고 라벨 없는 스냅샷은 최근 250개만 유지하며, 복원 전에는 커밋되지 않은 변경 사항을 먼저 자동 저장합니다.
...
### 오프라인 LaTeX 컴파일
Tectonic이 앱에 직접 내장되어 있습니다. 패키지는 처음 사용 시 한 번 다운로드되어 로컬에 캐시됩니다. 이후에는 TeX Live 설치 없이 완전히 오프라인으로 컴파일됩니다. 첫 컴파일 전에 번들이 이미 캐시되어 있는지 확인하고, 일회성 다운로드가 필요하면 온보딩에서 미리 알려주므로 오프라인 준비 상태가 갑자기 사라지는 일은 없습니다.
...
- **커리어 데이터베이스 & 이력서 신디사이즈** — 이력서를 드래그 앤 드롭으로 가져오고(`.tex` 또는 Overleaf식 `.zip`), PDF/Markdown/마인드맵에서 지식 베이스를 구축하고, 근거와 지표가 검증된 JD 맞춤형 이력서를 생성합니다. 내장 MCP 서버를 통해 외부 에이전트도 동일한 엔진을 사용할 수 있습니다 ([docs/PLUGINS.md](docs/PLUGINS.md)).
- **앱 내 업데이트** — 설정 → 환경에서 업데이트 상태 확인 및 원클릭 설치.
...
[GitHub Releases](https://github.com/bharathvbcr/DevPrism/releases)에서 최신 빌드를 다운로드하세요.
...
DevPrism은 [delibae](https://github.com/delibae)의 [claude-prism](https://github.com/delibae/claude-prism)에서 포크되었으며, claude-prism은 [assistant-ui](https://github.com/assistant-ui)의 [Open Prism](https://github.com/assistant-ui/open-prism)에서 시작되었습니다. 이 프로젝트는 수많은 훌륭한 오픈소스 프로젝트 위에 만들어졌습니다. 아래 프로젝트의 메인테이너와 커뮤니티에 깊이 감사드립니다:

**기반(Foundation)**
- [claude-prism](https://github.com/delibae/claude-prism) — [delibae](https://github.com/delibae). DevPrism이 직접 포크한 업스트림.
- [Open Prism](https://github.com/assistant-ui/open-prism) — [assistant-ui](https://github.com/assistant-ui). claude-prism의 기반이 된 원본 프로젝트.

**데스크톱 & UI**
- [Tauri](https://tauri.app) — Rust 기반 데스크톱 애플리케이션 프레임워크.
- [React](https://react.dev) + [Vite](https://vitejs.dev) — 프론트엔드 런타임 및 빌드 도구.
- [CodeMirror 6](https://codemirror.net) — LaTeX/BibTeX 소스 에디터.
- [Radix UI](https://www.radix-ui.com) & [Tailwind CSS](https://tailwindcss.com) — 컴포넌트 프리미티브 및 스타일링.

**과학 & 문서 엔진**
- [Tectonic](https://tectonic-typesetting.github.io) — 내장형 오프라인 LaTeX 엔진.
- [MuPDF](https://mupdf.com) — SyncTeX를 지원하는 네이티브 PDF 렌더링.
- [uv](https://docs.astral.sh/uv/) — [Astral](https://astral.sh). 내장 Python 환경을 구동하는 빠른 패키지 매니저.

**AI & 스킬**
- [Ollama](https://ollama.com) — 오프라인, 온디바이스 추론을 구동하는 로컬 LLM 런타임.
- [Anthropic Claude](https://www.anthropic.com/claude) — 선택형 클라우드 에이전트와 슬래시 명령어를 담당하는 어시스턴트.
- [K-Dense Scientific Skills](https://github.com/K-Dense-AI/claude-scientific-skills) — [K-Dense AI](https://github.com/K-Dense-AI). 100개 이상의 도메인별 과학 스킬.

**통합(Integrations)**
- [Zotero](https://www.zotero.org) — 참고문헌 관리 및 인용 삽입.

그리고 이 프로젝트가 의존하는 더 넓은 오픈소스 생태계의 라이브러리들에게도 감사드립니다. 🙏

## 라이선스

[MIT](./LICENSE) © 2026 delibae. 일부 © 2025 [assistant-ui](https://github.com/assistant-ui) (Open Prism).
