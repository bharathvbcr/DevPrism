...
DevPrism 是**本地优先**的替代方案 — 文件存储在本地磁盘，离线编译。AI 功能通过 Anthropic API 发送内容进行推理（参见[数据使用政策](https://code.claude.com/docs/en/data-usage)）。
...
### Python 环境（uv）
DevPrism 集成了 [uv](https://docs.astral.sh/uv/) — 快速的 Python 包管理器。一键安装 uv，一键创建项目级虚拟环境。Claude Code 在运行 Python 代码时自动使用 `.venv`，因此您可以在编辑器中直接生成图表、运行分析脚本和处理数据。
...
### 基于 Git 的历史记录
每次保存都会在本地 Git 仓库（`.claudeprism/history.git/`）中创建快照。标记重要检查点，浏览任意两个快照之间的差异，恢复以前的版本 — 全部在应用内完成。历史记录会自行管理体积：快照超过 800 个时自动压缩，带标签的检查点永久保留，无标签的仅保留最近 250 个；恢复前会先自动保存未提交的更改。

### 离线 LaTeX 编译
Tectonic 直接嵌入应用中。包在首次使用时下载一次并本地缓存。之后编译完全离线，无需安装 TeX Live。首次编译前会检查宏包缓存是否就绪，如需一次性下载会在引导流程中提前提示 — 离线可用性不会突然失效。
...
- **职业数据库与简历合成** — 拖放导入简历（`.tex` 或 Overleaf 风格的 `.zip`），从 PDF/Markdown/思维导图构建知识库，生成经过指标校验、贴合 JD 的简历。外部代理也可通过内置 MCP 服务器使用同一引擎（见 [docs/PLUGINS.md](docs/PLUGINS.md)）。
- **应用内更新** — 在 设置 → 环境 中查看更新状态并一键安装。
