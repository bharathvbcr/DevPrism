...
use crate::agent::{AgentState, Orchestrator};
use std::borrow::Cow;
...
#[serde(rename_all = "camelCase")]
pub struct ResumeKnowledgeSettings {
    pub resume_profile: String,
    pub manual_experience: String,
    pub evidence_entries: String,
}
...
fn create_command(
    ...
    // Remove all Dev Engine internal env vars to prevent nested session detection
    // and other interference. Tauri inherits these when launched from a Dev Engine session.
    cmd.env_remove("CLAUDECODE");
    cmd.env_remove("CLAUDE_AGENT_SDK_VERSION");
    for (key, _) in std::env::vars() {
        // Keep CLAUDE_CODE_GIT_BASH_PATH — Dev Engine needs it on Windows to locate git-bash
    ...
    // Dev Engine requires git-bash to run on Windows.
...
pub struct AgentCliStatus {
    ...
    /// Dev Engine requires git-bash to function on Windows.
...
async fn execute_native_agent(
    ...
    let project_state = window
        .state::<crate::agent::knowledge::ProjectState>()
        .inner()
        .clone();
    ...
    project_state
        .add_authorized_path(PathBuf::from(project_path))
        .await;

    let stored = merge_provider_settings(read_settings_value()?);
    let selected_provider = provider.unwrap_or_else(|| {
        if let Some(m) = model.as_deref() {
            if m == "ollama" || m.starts_with("ollama:") {
                return "ollama".to_string();
            }
            if m.starts_with("gemini") {
                return "gemini-api".to_string();
            }
        }
    ...
    let selected_model = model
        ...
        .or_else(|| Some(stored.model.clone()));
    let selected_backend = backend_mode.unwrap_or(stored.backend_mode.clone());
    ...
    let provider_impl: Arc<dyn Provider> = match selected_provider.as_str() {
        ...
        other => {
            return Err(format!(
                "Provider '{}' is not supported by the native agent backend '{}'.",
                other, selected_backend
            ));
        }
    };

    let mut orchestrator_inner = Orchestrator::new(provider_impl, project_state);
    orchestrator_inner.redact_secrets = get_redact_secrets().await.unwrap_or(true);
    orchestrator_inner.safe_mode = get_safe_mode().await.unwrap_or(true);
    let orchestrator = Arc::new(orchestrator_inner);
    orchestrators.insert(key, orchestrator.clone());
    ...
    tokio::spawn(async move {
        if let Err(err) = orchestrator
            .run_task(win_clone.app_handle().clone(), tab_id_clone.clone(), prompt)
            .await
        {
...
pub async fn continue_agent_code(
    ...
    if provider_choice == "gemini-api" || provider_choice == "ollama" {
        let agent_state = window.state::<AgentState>();
        let orchestrators = agent_state.orchestrators.lock().await;

        let window_label = window.label().to_string();
        let key = format!("{}:{}", window_label, tab_id);

        if let Some(orchestrator) = orchestrators.get(&key) {
            ...
            tokio::spawn(async move {
                if let Err(err) = orchestrator
                    .continue_task(win_clone.app_handle().clone(), tab_id_clone.clone(), prompt)
                    .await
                {
                    let _ = win_clone.emit(
                        "agent-error",
        ...
        // If no orchestrator found, start a new one
        drop(orchestrators);
        return execute_native_agent(
            window,
            project_path,
...
pub async fn kill_process_for_window(app_handle: &tauri::AppHandle, window_label: &str) {
    ...
    // Clean up native orchestrators
    let agent_state = app_handle.state::<AgentState>();
    let mut orchestrators = agent_state.orchestrators.lock().await;
    let keys_to_remove: Vec<String> = orchestrators
        .keys()
        .filter(|k| k.starts_with(&prefix))
        .cloned()
        .collect();
    for key in keys_to_remove {
        orchestrators.remove(&key);
    }
    drop(orchestrators);
...
/// Resolve the Dev Engine sessions directory for a given project path.
/// Dev Engine encodes paths by replacing all non-alphanumeric characters with '-'.
...
fn extract_first_user_message(path: &PathBuf) -> (Option<String>, Option<String>) {
    ...
    for line in reader.lines() {
        ...
        // Case 1: content is a plain string (Dev Engine stored JSONL format)
...
fn merge_provider_settings(settings: serde_json::Value) -> AgentProviderSettings {
    ...
    if let Some(url) = settings
        ...
        .filter(|value| !value.trim().is_empty())
    {
        defaults.ollama_base_url = url.to_string();
    }
    if let Some(model) = settings
        .get("ollamaModel")
        .and_then(|v| v.as_str())
        .filter(|value| !value.trim().is_empty())
    {
        defaults.ollama_model = model.to_string();
    }
    defaults
}

#[tauri::command]
pub async fn get_agent_provider_settings() -> Result<AgentProviderSettings, String> {
    Ok(merge_provider_settings(read_settings_value()?))
}

#[tauri::command]
pub async fn set_agent_provider_settings(settings: AgentProviderSettings) -> Result<(), String> {
    ...
    if let Some(obj) = value.as_object_mut() {
        obj.insert(
            "agentProvider".to_string(),
            serde_json::json!(settings.provider),
        );
        obj.insert("agentModel".to_string(), serde_json::json!(settings.model));
        obj.insert(
            "agentBackendMode".to_string(),
            serde_json::json!(settings.backend_mode),
        );
        obj.insert(
        ...
        obj.insert(
            ...
            serde_json::json!(settings.ollama_base_url),
        );
        obj.insert(
            "ollamaModel".to_string(),
            serde_json::json!(settings.ollama_model),
        );
    }
...
#[tauri::command]
pub async fn check_gemini_api_status(api_key: Option<String>) -> Result<ProviderHealth, String> {
    let settings = merge_provider_settings(read_settings_value()?);
    let has_key = api_key
        .filter(|key| !key.trim().is_empty())
        .or_else(|| settings.gemini_api_key.filter(|key| !key.trim().is_empty()))
        .or_else(|| {
            std::env::var("GEMINI_API_KEY")
                .ok()
                .filter(|key| !key.trim().is_empty())
    ...
    Ok(ProviderHealth {
        ok: has_key,
        message: if has_key {
            "Gemini API key is configured.".to_string()
        } else {
...
pub async fn check_ollama_status(
    ...
    let settings = merge_provider_settings(read_settings_value()?);
    let base = base_url.unwrap_or(settings.ollama_base_url);
    let selected_model = model
        .filter(|m| !m.trim().is_empty())
    ...
    let res = match client.get(&url).send().await {
        ...
        Err(e) => {
            return Ok(ProviderHealth {
                ok: false,
                message: format!("Could not reach Ollama at {}: {}", base, e),
                models: Vec::new(),
            });
        }
    };
    ...
    if models.is_empty() {
        return Ok(ProviderHealth {
            ok: false,
            message: format!(
                "Connected to Ollama at {}, but no local models are installed. Run `ollama pull <model>` first.",
                base
            ),
            models,
        });
    ...
    if !models.iter().any(|m| m == &selected_model) {
        return Ok(ProviderHealth {
            ok: false,
            message: format!(
                "Connected to Ollama at {}, but model '{}' is not installed.",
                base, selected_model
            ),
            models,
        });
    ...
    let tool_warning =
        crate::agent::providers::ollama::ollama_tool_capability_message(&selected_model);
    Ok(ProviderHealth {
        ok: true,
        message: format!(
            "Connected to Ollama at {} with model '{}'",
            base, selected_model
        ) + tool_warning
            .as_deref()
            .map(|warning| format!("\n{}", warning))
...
#[tauri::command]
pub async fn get_resume_knowledge_settings() -> Result<ResumeKnowledgeSettings, String> {
    let value = read_settings_value()?;
    let db_value = crate::agent::knowledge::cache::get_resume_knowledge()
    ...
    let read_string = |key: &str| {
        value
            ...
            .or_else(|| db_value.get(key).and_then(|v| v.as_str()))
            .unwrap_or("")
            .to_string()
    };
    Ok(ResumeKnowledgeSettings {
        resume_profile: read_string("resumeProfile"),
        manual_experience: read_string("manualExperience"),
        evidence_entries: read_string("evidenceEntries"),
    })
}

#[tauri::command]
pub async fn set_resume_knowledge_settings(
    settings: ResumeKnowledgeSettings,
) -> Result<(), String> {
    let mut value = read_settings_value()?;
    if !value.is_object() {
        value = serde_json::json!({});
    ...
    if let Some(obj) = value.as_object_mut() {
        obj.insert(
            "resumeProfile".to_string(),
            serde_json::json!(settings.resume_profile),
        );
        obj.insert(
            "manualExperience".to_string(),
            serde_json::json!(settings.manual_experience),
        );
        obj.insert(
            "evidenceEntries".to_string(),
            serde_json::json!(settings.evidence_entries),
        );
    }
    write_settings_value(&value)?;
    let _ = crate::agent::knowledge::cache::sync_resume_knowledge(
        None,
        Some(&settings.resume_profile),
        Some(&settings.manual_experience),
        Some(&settings.evidence_entries),
    );
    Ok(())
}
...
#[tauri::command]
pub async fn set_redact_secrets(enabled: bool) -> Result<(), String> {
    let mut settings = read_settings_value()?;
    if !settings.is_object() {
        settings = serde_json::json!({});
    }
    if let Some(obj) = settings.as_object_mut() {
        obj.insert("redactSecrets".to_string(), serde_json::json!(enabled));
    }
...
pub async fn set_safe_mode(enabled: bool) -> Result<(), String> {
    ...
    if let Some(obj) = settings.as_object_mut() {
        obj.insert("safeMode".to_string(), serde_json::json!(enabled));
    }
    write_settings_value(&settings)
}
...
pub async fn set_personal_bio(bio: String) -> Result<(), String> {
    ...
    if let Some(obj) = settings.as_object_mut() {
        if bio.is_empty() {
            obj.remove("personalBio");
        } else {
            obj.insert("personalBio".to_string(), serde_json::json!(bio));
        }
    }
    let content = serde_json::to_string_pretty(&settings)
        .map_err(|e| format!("Failed to serialize settings: {}", e))?;
    std::fs::write(&path, content).map_err(|e| format!("Failed to write settings: {}", e))?;
    let personal_bio = if bio.trim().is_empty() {
        None
    } else {
        Some(bio.as_str())
    };
