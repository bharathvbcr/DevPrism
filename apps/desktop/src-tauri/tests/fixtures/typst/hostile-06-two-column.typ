...
#set document(title: "```typ #read(\"/x\") ``` — Resume", author: "```typ #read(\"/x\") ```")
...
#{
doc-header("```typ #read(\"/x\") ```", ("```typ #read(\"/x\") ```", "```typ #read(\"/x\") ```", "```typ #read(\"/x\") ```"), (("", "```typ #read(\"/x\") ```"),))
two-col({
sect("Skills")
skill-line("```typ #read(\"/x\") ```", "```typ #read(\"/x\") ```")
}, {
sect("Summary")
para(rich(((false, "```typ #read(\"/x\") ```"),)))
sect("Experience")
entry("```typ #read(\"/x\") ```", "```typ #read(\"/x\") ```", "```typ #read(\"/x\") ```", "```typ #read(\"/x\") ```", "https://```typ #read(\"/x\") ```")
bullets((
  rich(((false, "```typ #read(\"/x\") ```"),)),
  rich(((false, "prefix ```typ #read(\"/x\") ``` suffix"),)),
))
extra(rich(((false, "```typ #read(\"/x\") ```"),)))
})
}
