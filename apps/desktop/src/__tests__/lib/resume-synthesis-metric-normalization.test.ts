...
describe("metricPreservedInText", () => {
  ...
  it("enforces word boundary on bare numbers to prevent false positives", () => {
    ...
    expect(metricPreservedInText("5", "5 squads delivered the feature")).toBe(
      true,
    );
    ...
    expect(metricPreservedInText("5", "Managed 15 direct reports")).toBe(false);
    expect(metricPreservedInText("5", "Reduced errors from 50 to 20")).toBe(
      false,
    );
  });
  ...
  it("handles percentage variations and formatting", () => {
    expect(
      metricPreservedInText(
        "25%",
        "Improved throughput by 25 percent across services",
      ),
    ).toBe(true);
    expect(metricPreservedInText("25%", "Improved throughput by 25 pct")).toBe(
      true,
    );
    expect(metricPreservedInText("25%", "Improved throughput by 25.0%")).toBe(
      true,
    );
    expect(
      metricPreservedInText(
        "25%",
        "Improved throughput by 25 percentage points",
      ),
    ).toBe(true);
  });
  ...
  it("handles currency and magnitude variations (K, M, B)", () => {
    ...
    expect(
      metricPreservedInText(
        "$1.2M",
        "Generated $1.2 million in annual recurring revenue",
      ),
    ).toBe(true);
    expect(metricPreservedInText("$1.2M", "Generated 1.2M USD in ARR")).toBe(
      true,
    );
    expect(
      metricPreservedInText("$1.2M", "Generated $1,200,000 in pipeline value"),
    ).toBe(true);
    expect(metricPreservedInText("$1.2M", "Generated $1200000 in ARR")).toBe(
      true,
    );
    ...
    expect(metricPreservedInText("$100K", "Saved 100k USD per quarter")).toBe(
      true,
    );
    expect(
    ...
    expect(
      metricPreservedInText(
        "$5B",
        "Oversaw transactions across a $5 billion portfolio",
      ),
    ).toBe(true);
  });
  ...
  it("handles multiplier / scale variations", () => {
    expect(
      metricPreservedInText(
        "5x",
        "Delivered a 5-fold speedup in query latency",
      ),
    ).toBe(true);
    expect(
      metricPreservedInText("5x", "Made batch processing 5 times faster"),
    ).toBe(true);
    expect(metricPreservedInText("5x", "Achieved 5X throughput gain")).toBe(
      true,
    );
  });
  ...
  it("handles comma-separated numbers and numeric abbreviations", () => {
    expect(
      metricPreservedInText(
        "10,000",
        "Serving over 10000 concurrent active users",
      ),
    ).toBe(true);
    expect(
...
describe("metricsValuesPreserved", () => {
  it("verifies all metrics in a list are present and preserved", () => {
    const metrics: import("@/lib/career/types").BulletMetric[] = [
      { value: "$1.2M", kind: "currency" },
      { value: "35%", kind: "percentage" },
      { value: "4", kind: "number" },
    ];
