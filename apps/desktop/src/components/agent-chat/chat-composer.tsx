...
import { useAgentChatStore, offsetToLineCol } from "@/stores/agent-chat-store";
import { useDocumentStore, type ProjectFile } from "@/stores/document-store";
import { useSettingsStore, type AgentProvider } from "@/stores/settings-store";
...
export const ChatComposer: FC<{ isOpen?: boolean }> = ({ isOpen }) => {
  ...
  const chatMode = useAgentChatStore((s) => s.chatMode);
  const setTabChatMode = useAgentChatStore((s) => s.setTabChatMode);

  const activeTabId = useAgentChatStore((s) => s.activeTabId);
  const activeTabSettings = useAgentChatStore(
    (s) => s.tabs.find((t) => t.id === activeTabId)?.agentProviderSettings,
  );
  const globalProviderSettings = useSettingsStore(
    (s) => s.agentProviderSettings,
  );
  const setTabProviderSettings = useAgentChatStore(
    (s) => s.setTabProviderSettings,
  );
  const useGlobalProviderSettingsForTab = useAgentChatStore(
    (s) => s.useGlobalProviderSettingsForTab,
  );
  const agentProviderSettings = activeTabSettings || globalProviderSettings;

  const effortLevel = useAgentChatStore((s) => s.effortLevel);
  const setEffortLevel = useAgentChatStore((s) => s.setEffortLevel);
  ...
  const resetProviderToGlobal = useCallback(() => {
    useGlobalProviderSettingsForTab(activeTabId);
    setModelPickerOpen(false);
  }, [activeTabId, useGlobalProviderSettingsForTab]);

  const toggleChatMode = useCallback(() => {
    setTabChatMode(
      activeTabId,
      chatMode === "linked-projects" ? "project" : "linked-projects",
    );
  }, [activeTabId, chatMode, setTabChatMode]);
  ...
  const handleSend = useCallback(() => {
    ...
    // Skills (scope === "skill") are passed through as-is; the active provider handles them via the Skill tool.
        ...
        createPortal(
          ...
          >
            ...
            <div className="p-1">
              ...
              {[
                {
                  id: "gemini-api" as const,
                  name: "Gemini API",
                  desc: agentProviderSettings.model,
                ...
                <button
                  ...
                  className={cn(
                    ...
                    agentProviderSettings.provider === m.id
      ...
      >
        ...
        <div className="flex items-center justify-between px-2 pb-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <TooltipIconButton
              tooltip={
                chatMode === "linked-projects"
                  ? "Ask across linked projects"
                  : "Current project only"
              }
              side="top"
              variant={chatMode === "linked-projects" ? "secondary" : "ghost"}
              size="icon"
