...
/**
 * Hook that manages Tauri event listeners for active agent streaming output.
 *
...
export function useAgentEvents() {
  ...
  useEffect(() => {
    ...
    function handleStreamMessage(payload: AgentOutputPayload) {
      ...
      // can interact with the widget before the agent continues.
      if (msg.type === "assistant" && msg.message?.content) {
    ...
    async function handleComplete(payload: AgentCompletePayload) {
      ...
      if (
        ...
        if (count === 0) {
          ...
          chatStore._setError(
            tabId,
            isWindows
              ? "Agent process failed to start. Check that Dev Engine CLI is installed and git-bash is available."
              : "Agent process failed to start. Check that Dev Engine CLI is installed.",
          );
        } else {
          chatStore._setError(
            tabId,
      ...
      // Auto-recompile after the agent finishes
      const {
        projectRoot,
        files,
        activeFileId,
    ...
    (async () => {
        ...
        (event) => {
          if (!cancelled) {
            ...
            if (
              ...
              !useAgentChatStore.getState().tabs.find((t) => t.id === tabId)
                ?.error
            ) {
              useAgentChatStore
                .getState()
                ._setError(
                  tabId,
                  "Dev Engine requires git-bash on Windows. Please install Git for Windows or set the CLAUDE_CODE_GIT_BASH_PATH environment variable.",
                );
            }
          }
        },
      ...
      const unlistenApproval = await listen<AgentApprovalPayload>(
        ...
        async (event) => {
          ...
          try {
            ...
            log.error("approval dialog failed", { error: String(err) });
          } finally {
            await invoke("resolve_agent_approval", {
              actionId,
              approved,
            });
          }
      ...
      listenersRef.current.push(unlistenApproval);
    })();
