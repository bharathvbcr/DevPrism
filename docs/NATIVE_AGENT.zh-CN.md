...
- **工具：** `Read`、`Write`、`Edit`（支持 `replace_all`）、`LS`、`Grep`（支持 `glob`/
  ...
  访问都限制在项目目录内。除这些内置工具外，代理还会公开插件注册表中精选的工具子集 —
  知识库检索、简历合成/差距分析/编译，以及受保护的简历文档编辑（`resume_doc_*`、
  `resume_variant_*`）。每个插件包按工具自行选择是否公开（参见 [PLUGINS.md](PLUGINS.md)）。
- **项目上下文：** 自动发现你的主文件/指令文件、项目地图以及已安装的技能
  （参见 [CONTEXT_FILES.md](CONTEXT_FILES.md)）。
- **记忆：** 按聊天标签页记住对话。
- **视觉：** 粘贴的图片会发送给支持视觉的模型（如 `llava`、`llama3.2-vision`）。
- **流式输出：** 文本/思考增量在跨 IPC 之前会在 Rust 中合并（约 40 毫秒或 1 KiB 窗口），
  输出碎片密集的本地模型也不会刷爆聊天界面。
